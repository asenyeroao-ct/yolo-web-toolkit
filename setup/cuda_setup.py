"""
YOLO Web Toolkit Environment Setup Script
Automatically creates virtual environment and installs all dependencies (including CUDA version of PyTorch)
"""
...
def install_pytorch_cuda():
    ...
    print_info("Installing PyTorch (CUDA 12.6)...")
    ...
    try:
        ...
        result = subprocess.run(
            [python_cmd, "-m", "pip", "install"] + packages + ["--index-url", pytorch_index],
            check=True
        )
        print()
        print_success("PyTorch (CUDA 12.6) installed successfully")
        return True
...
if torch.cuda.is_available():
    ...
    print("CUDA not available (PyTorch may be CPU-only or no GPU detected)")
"""
...
def main():
    ...
    if not install_pytorch_cuda():
        print_warning("PyTorch installation failed, but continuing...")
    print()
    ...
    print("=" * 60)
    print()
    print_info("Virtual environment created and configured")
