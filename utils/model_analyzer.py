...
def analyze_pt_model(model_path: str) -> ModelInfo:
    ...
    try:
        ...
        # 提取 YOLO 版本
        info.yolo_version = extract_yolo_version_from_pt(model_path, model)
        if info.yolo_version:
            info.metadata['yolo_version'] = info.yolo_version
...
def analyze_onnx_model(model_path: str) -> ModelInfo:
    ...
    try:
        ...
        # 提取 YOLO 版本
        info.yolo_version = extract_yolo_version_from_onnx(model_path)
        if info.yolo_version:
            info.metadata['yolo_version'] = info.yolo_version
...
def analyze_engine_model(model_path: str) -> ModelInfo:
    ...
    try:
        ...
        if trt_version:
            info.metadata['tensorrt_version'] = trt_version
